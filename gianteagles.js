...
(async () => {
  try {
    ...
    const url = "https://www.gianteagle.com/grocery/search?q=milk";
    ...
    await page.goto(url);
      ...
      const scrapeCurrentPage = async () => {
        ...
        const pageData = await page.evaluate(() => {
          const productElements = Array.from(
            document.querySelectorAll("div.ProductTile")
          );

          return productElements.map((product) => {
            const linkElement = product.querySelector("a");
            const titleElement = product.querySelector("div.sc-fbAgdq");
            const priceElement = product.querySelector("div.sc-kOGoUQ");
      ...
      const outputFilePath = path.join(__dirname, outputDirectory, "target_2.xlsx");
