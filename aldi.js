...
(async () => {
  try {
    ...
    const url = "https://www.aldi.us/en/products/dairy-eggs/milk-milk-substitutes/";
    ...
    await page.goto(url);
      ...
      const scrapeCurrentPage = async () => {
        ...
        const pageData = await page.evaluate(() => {
          const productElements = Array.from(
            document.querySelectorAll(".box--wrapper")
          );

          return productElements.map((product) => {
            const linkElement = product.querySelector("div.tx-aldi-products div.ym-grid a");
            const titleElement = product.querySelector("div.box--description--header");
            const priceElement = product.querySelector("span.box--baseprice");
      ...
      const outputFilePath = path.join(__dirname, outputDirectory, "aldi.xlsx");
